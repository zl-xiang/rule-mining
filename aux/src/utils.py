...
def is_empty(val:str)->bool:
    ...
    return (val == None or val in EMPTY_CONST or len(val)<1)
...
def is_sim(X,Y, threshold=85) -> bool:
    score = 0
    short_indicator = 200
    ...
    return int(score*100) >= threshold
...
# result = extract_eq_patterns(log_file_path)
# print(result)

# def normalize_tuple(t):
#     """Return a tuple in a consistent order to ensure symmetry."""
#     (tid1, pos1, tid2, pos2) = t
#     if (tid1, pos1) <= (tid2, pos2):
#         return (tid1, pos1, tid2, pos2)
#     else:
#         return (tid2, pos2, tid1, pos1)
...
if __name__ == "__main__":
    ...
    t2 = (3,4,1,2)
    t_lst = [t1_,t2_]
    ts = {normalize_tuple(t) for t in t_lst}
    #print(ts)
    #normalize_tuple()
    # generate_variants()
    opt_cache = './cache/lacep'
    aspen_sol_cache = './cache'
    fns_dir = f'./{opt_cache}/music-cell-50-cell-pos-sol.pkl'
    local_fns_dir = f'{opt_cache}/music-cell-wo-local-50-cell-pos-sol.pkl'
    #compare_cache(fname1,fname2)
    #compare_cache(fname2,fname1)
    fns = load_cache(fns_dir)
    fns_local = load_cache(local_fns_dir)
    # [print(e) for e in fns_local]
    #clingo_eq = {e+'.' for e in extract_eq_patterns('./heur-mu-eq.log')}
    diff = fns.difference(fns_local)
    [print(d) for d in diff]
    #print(len(fns))
    #print(len(fns_local))
    #[print(a) for a in diff]
    #print(len(mu_heur_eq))
    #print(len(mu_asprin_eq))
    #print(len(eqvs))
    #[print(e) for e in diff]
    #sims =  load_cache('./cache/lacep/sim-music-cell50-cell.pkl')
    #mucc_atom = load_cache('./cache/lacep/atoms_musicbrainz_ds_50-cell.pkl')
    #atoms = load_cache('./cache/lacep/atoms_imdb-trc.pkl')

    #[print(a) for a in mucc_atom] 
    #[print(a) for a in sims]
    #print(len(sims))
    #[print(e) for e in sims]
    #ubs = load_cache('./cache/lacep/music-plus-50-cell-ub.pkl')
    #print(len(ubs))
    #[print(e) for e in ubs]
    #fname1 = 'imdb-4k'
    ...
    #s = (119.99 )
    #print('p')
    ##price_convert(p,VOUCHER_RATE,GBP2RMB_RATE)
    #print('x 1')
    #price_convert(x,VOUCHER_RATE,GBP2RMB_RATE)
    #print('s')
    #price_convert(s,VOUCHER_RATE,GBP2RMB_RATE)
    # example 2
    #print(semantic_sim('Association of Tennis Professionals',"ATP"))
    # print(sim('Drama-Short',"Drama"))
