...
from . util import order_prog, prog_is_recursive, rule_is_recursive, calc_rule_size, calc_prog_size, prog_hash, format_rule, format_literal, Literal, is_int, reorder_body
...
class Tester():
    ...
    def test_prog_all(self, prog):
        ...
        if len(prog) == 1:
            ...
            atom_str, body_str = self.parse_single_rule(prog)
            q = f'findall(_ID, (pos_index(_ID, {atom_str}), ({body_str}->  true)), S)'
            self.settings.logger.debug(f"[Testing] query pos started q = {q}")
            pos_covered = query_once(q)['S']
            self.settings.logger.debug(f"[Testing] query pos ended")
            neg_covered = []
            if self.num_neg > 0:
                self.settings.logger.debug(f"[Testing] query neg started")
                q = f'findall(_ID, (neg_index(_ID, {atom_str}),({body_str}->  true)), S)'
                
                neg_covered = query_once(q)['S']
                self.settings.logger.debug(f"[Testing] query neg ended")
        else:
            with self.using(prog):
