...
class Popper():
    ...
    def run(self, bkcons):
        # TP:3 FN:0 TN:0 FP:0 only 
        # Complete = 0 FN and TP > 0
        # Consistent = 0 FP
        ...
        for size in range(2, max_size+1):
            ...
            while True:
                ...
                # execute if prog is not subsumed by existing good rules and does not subsume failed rules
                ...
                with settings.stats.duration('test'):
                    ...
                    # settings.logger.debug(f'*** strange type {type(pos_covered)} {type(neg_covered)}')
                ...
                found_more_specialised = False
                # settings.logger.debug(f'*** prog covered {pos_covered} --- {neg_covered}')
                if covers_pos:
                    ...
                    # if from crtieria it is a good rule, check if we recorded anything worse or better
                    if not inconsistent and settings.solution and len(to_combine)>0:
                        
                        for p in settings.solution.copy():
                            # settings.logger.debug(f'*** [Check existing] exists seen_rule {format_rule(p)}, current prog {prog_core_str}')
                            hash_p = hash(frozenset({tuple(p)}))
                            p_pos_covered = coverage_pos[hash_p]
                            p_neg_covered = coverage_neg[hash_p]
                            # settings.logger.debug(f'*** [Conditions] hash_p in to_combine {hash_p in to_combine}, prog_core>p {rule_subsume_trc(prog_core,p,range_pair)}, p>prog_core {rule_subsume_trc(p,prog_core,range_pair)}')
                            
                            ## if current rule is good and more general than a recorded rule, check whether current rule cover no more negs than recorded
                            ### if no, prune generalisations of current rule do not add to combiner
                            ### if yes, keep the current rule, prune its specalisations and abandon the recorded, add to combiner
                            # settings.logger.debug(f'*** strange type {type(p_pos_covered)} {type(p_neg_covered)}')
                            if hash_p in to_combine and rule_subsume_trc(prog_core,p,range_pair):
                                settings.logger.debug(f'*** neg covered type {type(p_neg_covered)} {type(neg_covered)}')
                                # settings.logger.debug(f'*** pos covered diff {pos_diff}')
                                # settings.logger.debug(f'*** neg covered diff {neg_diff}')
                                # settings.logger.debug(f'*** pos covered diff {pos_diff}')
                                # neg_diff = neg_covered ^ p_neg_covered
                                # pos_diff = pos_covered ^ p_pos_covered
                                # make sure it is more general in a good way
                                #if neg_covered == p_neg_covered:
                                self.settings.logger.debug(f'*** [found more general] exists seen_rule {format_rule(p)} > prog {prog_core_str}, and seen_rule [covers pos] and [consistent] ')
                                to_combine.remove(hash_p)
                                settings.solution = set(settings.solution)
                                settings.solution.remove(p)
                                settings.solution = frozenset(settings.solution)
                                found_more_general = True
                                #else:
                                 #   inconsistent = True
                            ## if current rule is good and more specific than a recorded rule, check whether current rule cover as many pos than recorded
                            ### if yes, keep the current rule, prune its abandon the recorded, add to combiner
                            ### if not, keep the current rule, prune its specalisations and abandon the recorded, add to combiner
                            # elif hash_p in to_combine and rule_subsume_trc(p,prog_core,range_pair):
                            #     self.settings.logger.debug(f'*** [found more specialised] prog {prog_core_str}  > exists seen_rule {format_rule(p)}, and seen_rule [covers pos] and [consistent] ')
                            #     self.settings.logger.debug(f'*** covers {pos_covered == p_pos_covered}')
                            #     if pos_covered == p_pos_covered:
                            #         # self.settings.logger.debug(f'*** [found more specialised] prog {prog_core_str}  > exists seen_rule {format_rule(p)}, and seen_rule [covers pos] and [consistent] ')
                            ...
                            #         settings.solution.remove(p)
                            #         settings.solution = frozenset(settings.solution)
                            #         found_more_specialised = True
                            #     else:
                            #         add_spec = True
                                
                        if found_more_general:
                            add_spec = True
                            #settings.solution = settings.solution | {prog_core}
                            prog = frozenset({tuple(prog_core)})
                            #to_combine.add(hash(prog))

                        # if found_more_specialised:
                        #     add_gen = True
                        #     #settings.solution = settings.solution | {prog_core}
                        #     prog = frozenset({tuple(prog_core)})
                ...
                # if consistent and fn> 0, prune specialisations
                if not inconsistent and not skipped:
                    add_spec = True

                # if covers all positive examples and inconsistent prune generalisations
                if tp == num_pos:
                    add_gen = True
                ...
                if not pruned_more_general:
                    if inconsistent:
                        ...
                        neg_covered = frozenbitarray(len(pos_covered))
                ...
                if not inconsistent and not add_gen and tp > 0 :
                #or (not inconsistent and found_more_specialised and tp > 0):
                #and not found_more_general:
...
def head_connected(rule):
    ...
    while body_literals:
        connected = []
        for literal in body_literals:
            # base case only the head vars
            # then add new vars as they appear in literals connected to head
        ...
        # if for any iteration there is a variable disjoint from all connected vars, return false
        if not connected and body_literals:
            return False
        # remove those already checked are connected
