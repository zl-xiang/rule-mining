...
class Generator:
    ...
    def __init__(self, settings, bkcons=[]):
        ...
        sim_thresh_patt = re.compile(r"sim_thresh\(\s*(\d+)\s*\)\.")


        schema_attr_tups = []
        sim_thresholds = []
        for match in sim_thresh_patt.finditer(bias_text):
            sim_thresholds.append(int(match.group(1)))  
        if len(sim_thresholds) <1:
            sim_thresholds = [90]
        ...
        for arity, xs in vars_list:
            for att_tup in schema_attr_tups:
                ...
                if arity == 3 and int(att_tup[-1])!= 0:
                        ...
                        _xs = xs
                        xs_vars = sorted((0,_xs[0],_xs[2]))
                        _xs = list(_xs)
                        _xs[1] = att_tup[0]
                        xs_str = ', '.join(str(x) for x in _xs)
                        encoding.append(f'vars({arity}, ({xs_str})).')
            ...
            ## [20260402] added fixed constant for similarity
            for theta in sim_thresholds:
                if arity == 3 :
                    xs_vars = sorted((0,xs[0],xs[1]))
                    xs = list(xs)
                    xs[2] = theta
                    xs_str = ', '.join(str(x) for x in xs)
                    encoding.append(f'vars({arity}, ({xs_str})).')
                    for i in range(len(xs)-1):
                        encoding.append(f'var_pos({xs[i]}, ({xs_str}), {i}).')
                    encoding.append(f'const_pos({xs[2]}, ({xs_str}), 2).')
        ...
        for k in range(max_arity):
            ...
            elif max_arity == 3 and k == 2:
                skip_const_pos_lower = f', not const_pos(V{k}, Vars1, 2), Vars1 = ({xs1}), not const_pos(X{k},Vars2,2), Vars2 = ({xs2})'
            order_cons.append(f'lower(({xs1}),({xs2})):- var_tuple(({xs1})), var_tuple(({xs2})), X{k} < V{k} {skip_const_pos_lower}.')
        # a skipped variable V from Vars1 is witness by a tuple Vars2 with lower rank 
        for k in range(max_arity-1):
            ...
            elif max_arity == 3 and k == 1:
                skip_const_pos_seen_lower = ', not const_pos(V2, Vars1,2), not const_pos(V, Vars2, 2)'
                # skip_const_pos_gap_ = ''
            ...
            # its ok here, v0, v1 wont be constant since var_tuple are the ordered var facts 
        ...
        if max_arity == 3:
            ...
            # when V1 is a const_pos, skipped variables are compared from V0 and V2
            # [Leon] !!! this rule never applies? V1 must not be a const position since var_tuple are taken from ordered_vars
    ...
    def build_generalisation_constraint3(self, prog, size=None):
        ...
        # [20260404] too complicated to generate all combinations of different sim threshold, use subsumption order to filter instead
        ...
        # print(rule, ' ===========')
        # here gen_rules is a set of normal rules
        for i,gr in enumerate(gen_rules):
            # [20260404] Leon: hacky here, assuming int represents variables do not exceed 50 
            # and sim thresholds are not lower than 50
            # all_bvars = {x for lit in gr[1] for x in lit.arguments if is_int(x) and x<50}
            # next_var = max(all_bvars)+1
            self.settings.logger.debug(f"%%% gen cons no. {str(i)}")
            # gr = remap_variables(gr)
            body_len = len([lit for lit in gr[1] if lit.predicate!='sim'])
            # new_body = set()
            # var_thresh_lookup = dict()
            # for lit in gr[1]:
            #     # replace sim constants with variables
            #     if lit.predicate =='sim':
            #         new_args = list(lit.arguments[:-1])
            #         new_args.append(next_var)
            #         var_thresh_lookup[next_var] = lit.arguments[-1]
            #         next_var+=1
            #         new_atom = Literal(lit.predicate, tuple(new_args))
            #         new_body.add(new_atom)
            #     else:
            #         new_body.add(lit)
            # new_rule = (gr[0],frozenset(new_body))
            # new_rule = remap_variables(new_rule)
            # find_variants converts normal rules to meta rules with variable shuffles
    ...
    #         #     sim_vars.add(lit.arguments[-1])
    #         #     body_vars.add(lit.arguments[-1])
    ...
    #             # if pred == 'sim':
    #             #     if cons_type == Constraint.GENERALISATION:
    #             #         (True, 'body_literal', (0, pred, len_new_args, new_args))
