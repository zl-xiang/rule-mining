...
def is_similarity_atom(lit: Literal) -> bool:
    """
    Similarity atom:
    sim(var1, var2, threshold)
    """
    return lit.predicate == "sim" and len(lit.arguments) == 3
...
def comp(rule: Rule) -> Set[Comparison]:
    ...
    attrs = [lit for lit in body if is_attribute_atom(lit)]
    ...
    for a1, a2 in combinations(attrs, 2):
        ...
        if v1 == v2 and (t1 != t2 or att1 != att2):
            # canonical ordering to avoid symmetry
            (x1, y1), (x2, y2) = sorted(
                [(t1, att1), (t2, att2)],
                key=lambda x: (x[0], x[1])
            )
            comparisons.add((x1, y1, x2, y2, 100))

    # ----------------------
    # Similarity (thresh < 100)
    # ----------------------
    sim_atoms = [lit for lit in body if is_similarity_atom(lit)]

    for sim in sim_atoms:
        v1, v2, thresh = sim.arguments
        ...
        for a1 in A1:
            for a2 in A2:
                ...
                comparisons.add((x1, y1, x2, y2, thresh))
...
def expand_rule_with_sim(rule, attr_bias:dict=None, sim_defined:set = None):
    """
    ...
    for lit in body:
        if lit.predicate == 'att':
            ...
            is_sim_defined = any([attr in sim_def for sim_def in sim_defined]) if sim_defined!=None else True
            if is_sim_defined:
    ...
    for val_var, att_list in value_var_groups.items():
        if len(att_list) >= 2:
            attr_type = attr_bias[att_list[0][1]]
            # is_sim_defined = any([a in sim_def for a in att_list[0][1] for sim_def in sim_defined]) if sim_defined!=None else True
            # if not object attribute and not tid
            if attr_type != 2 and attr_type != 0:
...
def meta_atoms_to_cons(meta_atoms):
    ...
    Given a set of meta-atoms, return an ASP constraint string of the form:
        :- b1, b2, ..., bn, V0 != V1, V0 != V2, ...
    where all variables V* occurring in the meta-atoms are required to be distinct.
    """
    ...
    for atom in meta_atoms:
        body_atoms.append(atom_to_asp(atom))
        all_vars |= collect_vars(atom)

    # pairwise inequality constraints
    neq_atoms = [f"{v1} != {v2}" for v1, v2 in combinations(sorted(all_vars), 2)]

    full_body = body_atoms + neq_atoms

    return ":- " + ", ".join(full_body) + "."
...
def atoms_ranged_by_comparisons(rule, comparisons):
    """
    Given a rule and a set of comparisons, return the set of atoms
    (relational, attribute, and similarity) participating in them.
    ...
    # --- index relational atoms (arity 1, non-att) ---
    rel_atoms = {
        lit.arguments[0]: lit
        for lit in body
    ...
    # --- index attribute atoms by (tid, att_name) ---
    att_atoms = {}
    for lit in body:
        if lit.predicate == 'att':
            tid, att_name, var = lit.arguments
    ...
    # --- index similarity atoms by variable pair (unordered) ---
    sim_atoms = {}
    for lit in body:
        if lit.predicate == 'sim':
            v1, v2 = lit.arguments
            key = tuple(sorted((v1, v2)))
            sim_atoms.setdefault(key, []).append(lit)

    ranged_atoms = set()

    for (t1, att1, t2, att2, comp_type) in comparisons:
        # ---- attribute atoms ----
        ...
        # ---- similarity atoms (only if requested) ----
        if comp_type == 1:
            # collect all similarity atoms induced by these attributes
            for a1 in att_atoms.get((t1, att1), []):
                for a2 in att_atoms.get((t2, att2), []):
                    v1 = a1.arguments[2]
                    v2 = a2.arguments[2]
                    key = tuple(sorted((v1, v2)))
                    for sim in sim_atoms.get(key, []):
                        ranged_atoms.add(sim)
...
def normalise_body_var(rule, available_vars)->Rule:
    """
    Replace body variables > len(body) with fresh variables from available_vars
    that are < len(body) and do not clash with existing body variables.

    All occurrences of the same variable are replaced consistently.
    """
    head, body = rule
    

    # Step 1: collect all variables appearing in the body
    body_vars = set()
    ...
    for lit in body:
        if lit.predicate == 'att':
            ...
            # relational atom
            body_vars.add(lit.arguments[0])
            
    body_var_size = len(body_vars)
    # Step 2: variables that need replacement
    to_replace = sorted(v for v in body_vars if v > body_var_size-1)

    # Step 3: variables already in use and valid
    used_vars = {v for v in body_vars if v < body_var_size}

    # candidate fresh variables
    candidates = [
        v for v in available_vars
        if v < body_var_size and v not in used_vars
    ]
    #print('cand',candidates)
    #print('to replace', to_replace)
    
    if len(candidates) < len(to_replace):
        raise ValueError("Not enough fresh variables to replace body variables.")

    # Step 4: build replacement mapping
    replacement = dict(zip(to_replace, candidates))

    # Step 5: apply mapping to body
    new_body = set()
    ...
    for lit in body:
        if lit.predicate == 'att':
            tid, att, var = lit.arguments
            new_args = (
                replacement.get(tid, tid),
                att,
                replacement.get(var, var)
            )
            new_body.add(Literal(lit.predicate, new_args))

        elif lit.predicate == 'sim':
            v1, v2 = lit.arguments
            new_args = (
                replacement.get(v1, v1),
                replacement.get(v2, v2)
            )
            new_body.add(Literal(lit.predicate, new_args))
            ...
            # relational atom
            tid = lit.arguments[0]
            new_args = (replacement.get(tid, tid),)
            new_body.add(Literal(lit.predicate, new_args))

    return (head, frozenset(new_body))


def collect_sim_var_pairs(rule):
    """
    Collect (v1, v2) from sim(v1, v2) where:
    - v1 != v2
    - both occur in attribute atoms
    """
    _, body = rule

    # collect all value variables from attribute atoms
    att_vars = set()
    for lit in body:
        if is_attribute_atom(lit):
            _, _, v = lit.arguments
    ...
    for lit in body:
        if lit.predicate == "sim":
            v1, v2 = lit.arguments
            if v1 != v2 and v1 in att_vars and v2 in att_vars:
                pairs.append((v1, v2))

    return pairs

def unify_fol_var(rule, v_from, v_to):
    head, body = rule
    mapping = {v_from:v_to}
    def subst_lit(lit):
        new_args = tuple(mapping.get(arg, arg) for arg in lit.arguments)
        return Literal(lit.predicate, new_args)
    new_body = frozenset(subst_lit(lit) for lit in body)
    return (head, new_body)


def get_sim_symmetries(rule):
    """
    Given a rule (head, body), generate all similarity symmetries
    by independently flipping sim(u,v) -> sim(v,u).

    Returns a set of rules (head, frozenset(body)).
    The original rule is NOT included.
    """
    head, body = rule
    body = list(body)

    # collect indices of sim atoms that can be flipped
    sim_indices = []
    for i, lit in enumerate(body):
        if lit.predicate == 'sim':
            u, v, _ = lit.arguments
            if u != v:   # flipping sim(v,v) is pointless
    ...
    # each sim atom independently flipped or not
    for choice in product([0, 1], repeat=k):
        if all(c == 0 for c in choice):
            continue  # skip identity (original rule)

        new_body = []
        for i, lit in enumerate(body):
            if i in sim_indices:
                idx = sim_indices.index(i)
                if choice[idx] == 1:
                    u, v, thresh = lit.arguments
                    new_body.append(
                        Literal('sim', (v, u, thresh))
                    )
                else:
                    new_body.append(lit)
            else:
...
def parse_rule(rule_string):
    ...
    Variables must be written as V<number>.
    """
    ...
    if ":-" in rule_string:
        ...
        body_literals = split_literals(body_str)
    else:
        head_str = rule_string
        body_literals = []

    head = parse_literal(head_str.strip())
    body = {parse_literal(lit.strip()) for lit in body_literals}
    body = frozenset(body)
...
def split_literals(body_str):
    """
    Split body string into literal strings,
...
def parse_literal(literal_str):
    """
    Parse a literal like att(V2,primaryTitle,V4)
    """
    match = re.match(r"([a-zA-Z_][a-zA-Z0-9_]*)\((.*)\)", literal_str)
    if not match:
