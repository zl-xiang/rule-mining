...
def format_literal(literal):
    ...
    args_formated = []
    for i in args:
        if is_int(i) and int(i)<50:
            args_formated.append(f'V{i}')
        else:
            args_formated.append(str(i))
    args = ','.join(args_formated)
    return f'{pred}({args})'


def format_sim(literal,index):
    pred, args = literal
    args_formated = []
    threshold_lit = ''
    for i in args:
        if is_int(i) and i<50:
            args_formated.append(f'V{i}')
        else:
            args_formated.append(f'S{str(index)}')
            threshold_lit = f'S{str(index)}>={str(i)}'
    args = ','.join(args_formated)
    return f'{pred}({args}), {threshold_lit}'
...
def format_rule2(rule):
    head, body = rule
    head_str = ''
    if head:
        head_str = format_literal(head)
    sim_lits = [lit for lit in body if lit[0] == 'sim']
    body_str = ','.join(format_literal(literal) for literal in body if literal[0] != 'sim')
    if len(sim_lits)>0:
        sim_str = ','.join(format_sim(literal,i) for i,literal in enumerate(sim_lits))
        body_str = ','.join([body_str,sim_str])
    return f'{head_str}:- {body_str}.'
...
class Settings:
    def __init__(self, cmd_line=False, info=True, debug=False, show_stats=True, max_literals=MAX_LITERALS, timeout=TIMEOUT, quiet=False, eval_timeout=EVAL_TIMEOUT, max_examples=MAX_EXAMPLES, max_body=None, max_rules=None, max_vars=None, functional_test=False, kbpath=False, ex_file=False, bk_file=False, bias_file=False, showcons=False, no_bias=False, order_space=False, noisy=False, batch_size=BATCH_SIZE, solver='rc2', anytime_solver=None, anytime_timeout=ANYTIME_TIMEOUT):
        ...
        for x in solver.symbolic_atoms.by_signature('sim_thresh', arity=1):
            sim_thresh.append(int(str(x.symbol.arguments[-1])))
        if len(sim_thresh) == 0:
        ...
        for x in solver.symbolic_atoms.by_signature('sim_thresh', arity=1):
            for k, args in cached_atom_copy.items():
                if len(k) == 3:
                    theta = x.symbol.arguments[0]
                    args_sym_index = (k[0],k[1],theta)
                    att_tuple = (args[0], args[1], theta.number)
                    self.cached_atom_args[args_sym_index] = att_tuple
            # self.schema_attrs[x.symbol.arguments[0].name] = x.symbol.arguments[1].number
        ...
        for pred, arity in self.body_preds:
            for k, args in self.cached_atom_args.items():
                if len(args) != arity:
                    continue
                if len(args) == 3:
                    if not is_int(args[1]) and  pred =='sim':
                        continue
                    elif pred == 'att' and is_int(args[1]):
                        continue
    ...
    def print_incomplete_solution2(self, prog, tp, fn, tn, fp, size):
        ...
        for rule in order_prog(prog):
            i+=1
            self.logger.info(format_rule2(self.order_rule(rule)))
    ...
    def print_prog_score(self, prog, score):
        ...
        for rule in order_prog(prog):
            print(format_rule2(self.order_rule(rule)))
        # print(self.format_prog(order_prog(prog)))
        print('*'*30)
...
def format_prog(prog):
    return '\n'.join(format_rule2(rule) for rule in prog)
...
def format_rule_symmetry_breaking(compatible_attrs,rule) -> str:
    """
    For each pair of distinct attribute value variables coming from
    incompatible attribute names, append V_small < V_large
    to the rule body.
    """
    ...
    # Collect (attr_name, value_var) pairs
    attr_values = []

    for lit in body:
        if lit.predicate == 'att':
            _, attr_name, value_var = lit.arguments
            if isinstance(value_var, int):
                attr_values.append((attr_name, value_var))


    compatible = compatible_attrs
    # for a1, a2 in compatible_attrs:
    #     compatible.add((a1, a2))
    #     compatible.add((a2, a1))

    sym_breaking_lits = set()

    # Check all pairs
    for (attr1, V1), (attr2, V2) in combinations(attr_values, 2):

        if V1 == V2:
            continue

        if (attr1, attr2) in compatible:
            continue

        # enforce canonical ordering by variable offset
        left = min(V1, V2)
        right = max(V1, V2)

        lt_literal = f'V{left} <> V{right}'
        sym_breaking_lits.add(lt_literal)

    rule_str = format_rule(rule)[:-1]+','
    rule_str += ','.join(sym_breaking_lits)+'.'
    rule_str += '\n eqo(X,Y):-eqo(Y,X).'
