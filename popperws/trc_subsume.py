...
def rule_subsume_trc(r1, r2, range_pair:tuple[tuple[str,str],tuple[str,str]])->bool:
    ...
    for mapping in all_mappings:
        ...
        if range_pair not in range_pair_r1h:
            ...
            _,body_r1h = r1h
            # print(f'**** checking {format_rule(r1)},\n {format_rule(r2)}')
...
def containment_check(c1h:set,c2:set)-> bool:
    ...
    for a1,a2 in product(c2_atts,repeat=2):
        for sim in c2_sim_atoms:
            ...
            if a1.arguments[-1] == sim.arguments[0] and a2.arguments[-1] == sim.arguments[1]:
                ...
                # [20260403] add extra arguments of similarity
                c2_sim_joins.add((a1.arguments[:-1],a2.arguments[:-1],sim.arguments[-1]))
    # print('c2 joins', c2_joins)
    ...
    # otherwise, check attribute joins containment
        ...
        for tup1 in c1h_tups:
            for tup2 in c1h_tups:
                if tup1[0] == 'att' and tup2[0] == 'att' and tup1[1][:-1] != tup2[1][:-1]:
                    ...
                    var2 = tup2[1][2]
                    if var1 == var2:
                        # print(tup1[1][:-1],tup2[1][:-1])
                        included_flag = (tup1[1][:-1],tup2[1][:-1]) in c2_joins or (tup2[1][:-1],tup1[1][:-1]) in c2_joins
                        # if there is a pair of attribute joins in c1h not in c2, then c1 \not subsumes c2
                        if not included_flag: return False

        
        # TO REMOVE
        # print(f'**** join check passed')
        ...
        for tup1 in c1h_tups:
            if tup1[0] == 'sim':
                # [20260403] [TODO] for a sim atom in c1h, check first if there is a sim atom over the same set of attribute pairs in c2 that holds greater threshold
                # otherwise check similarity and join
                ...
                # [20260403] [TODO] need to check above with an example. 
                ###  should be every pair of sim (since sim may over different attribute pairs) there is a join?
                sim_var1 = tup1[1][0]
                sim_var2 = tup1[1][1]
                thresh = tup1[1][2]
                thresh = int(thresh)
                ...
                # print(' sim var range', sim_var_range)
                pairs  = product(sim_var_range[sim_var1],sim_var_range[sim_var2])
                pairs = set(pairs)
                # print(' pairs', pairs)
                checked = set()
                for p in pairs:
                    if (p[0],p[1]) in c2_joins or (p[1],p[0]) in c2_joins:
                        checked.add(p)
                # print('sim join containment checked', checked)
                to_check = set(pairs) - checked
                # print('sim containment to be checked', to_check)
                if len(to_check)!= 0:
                    for p in to_check:
                        for sim_pair in c2_sim_joins:
                            # print((p[0],p[1]), sim_pair[:-1])
                            if (p[0],p[1]) == sim_pair[:-1] or (p[1],p[0]) == sim_pair[:-1]:
                                # print('sim pair coincide', p[0],p[1])
                                #print('----', sim_pair[-1])
                                if thresh<= int(sim_pair[-1]):
                                    # print('threshold checks passed', thresh, sim_pair[-1])
                                    checked.add(p)
                to_check = set(pairs) - checked
                
                included_flag = len(to_check) == 0
...
def apply_mapping(c1,mapping,max_filling_vars=100, head_vars:tuple[int,int] = None)->tuple[tuple,set]:
    ...
    for lit1 in c1:
        if lit1.predicate == 'att':
            ...
            new_lit1_args = (f'{str(lit1.arguments[0])}{mark1}', f'{str(lit1.arguments[1])}{mark2}', f'{lit1.arguments[-1]}')
            new_c1.add((lit1.predicate,new_lit1_args))
