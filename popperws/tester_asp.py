...
from . util import order_prog, prog_is_recursive, rule_is_recursive, calc_rule_size, calc_prog_size, prog_hash, format_rule,format_rule2, format_literal, Literal, is_int, reorder_body, format_prog, format_rule_symmetry_breaking
...
# EQ_PROP = """
# eqo(X,Y):- eqo(Y,X).
# eqo(X,Y):- eqo(X,Z), eqo(Y,Z).
# """

POS = """
pos_covered(ID):- eqo(X,Y), pos(eqo(X,Y),ID).
pos_covered(ID):- eqo(X,Y), pos(eqo(Y,X),ID).
"""

NEG = """
neg_covered(ID):- eqo(X,Y), neg(eqo(X,Y),ID).
neg_covered(ID):- eqo(X,Y), neg(eqo(Y,X),ID).
"""
...
class Tester():
    ...
    def __init__(self, settings):
        ...
        # %%%%%%%%%%%% clingo %%%%%%%%%%%%%%
        self.pos_ex, self.neg_ex = get_indexed_examples(exs_pl_path)
         
        self.num_pos = len(self.pos_ex)
        self.num_neg = len(self.neg_ex)
        # print(self.num_pos)
        # print(self.num_neg)
    ...
    def parse_single_rule(self, prog):
        rule = next(iter(prog))
        head = rule[0]
        ordered_body = reorder_body(rule[1])
        # ordered_body = self.settings.order_rule(rule)
    ...
    def test_prog(self, prog):
        ...
        if self.settings.recursion_enabled or self.settings.pi_enabled:
            ...
            if len(prog) == 1:
                ...
                if inconsistent:
                    q_neg = f'findall(_ID, (neg_index(_ID, {atom_str}), ({body_str} ->  true)), S)'
                    neg_covered = query_once(q_neg)['S']
            else:
        ...
        return pos_covered, inconsistent, neg_covered

    def test_prog_all(self, prog, is_combine = False):
        solver = clingo.Control(['-Wnone'])
        with open(self.settings.bk_file) as f:
            solver .add('base', [], f.read())
        exs_str = '\n'.join(self.pos_ex) + '\n'.join(self.neg_ex)
        # print(exs_str+"#show pos_covered/1.\n"+"#show neg_covered/1.\n")
        solver .add('base', [], exs_str+"#show pos_covered/1.\n"+"#show neg_covered/1.\n")
        solver .add('base', [], "sim(X,X,100):- att(_,_,X).\n"+"sim(X,Y,T):- sim(Y,X,T). sim(X,Y,T'):- sim(X,Y,T), sim_thresh(T'), T>T'.")
        solver.ground(((('base',[])),))
        self.settings.logger.debug(f"[Testing] started")
        if len(prog) == 1:
            rule = next(iter(prog))
            rule_str = format_rule2(rule)
            #comp_attr = self.settings.comp_attr
            #rule_str = format_rule_symmetry_breaking(comp_attr,rule)
        else:
            rule_str = format_prog(prog)
        self.settings.logger.debug(f"[Testing] query examples started q = {rule_str}")
        solver .add('test',[],POS+NEG)
        solver .add('test',[],rule_str)
        if is_combine: solver .add('test',[],'eqo(X,Y):-eqo(Y,X).\n eqo(X,Z):-eqo(X,Y),eqo(Y,Z).\n eqo(X,X):-att(T,A,X),target(_,A,_).')
        solver.ground(((('test',[])),))
        pos_covered = []
        neg_covered = []
        with solver.solve(yield_=True) as hnd:
            for m in hnd:
                for s in m.symbols(shown=True):
                    if s.name == 'pos_covered':
                        #print(s.arguments)
                        pos_covered.append(int(s.arguments[0].number))
                    elif self.num_neg>0:
                        # print(s.arguments)
                        neg_covered.append(int(s.arguments[0].number))

        self.settings.logger.debug(f"[Testing] query examples ended")
        del solver
...
def get_indexed_examples(file_path):
    """
    Parse a file and extract:
        pos(eqo(const1,const2))
        neg(eqo(const1,const2))
    Then expand them to:
        pos(eqo(const1,const2),id)
        neg(eqo(const1,const2),id)

    Returns:
        (set_of_indexed_pos_facts, set_of_indexed_neg_facts)
    """

    # pattern for:
    # - lowercase identifiers: [a-z][a-zA-Z0-9_]*
    # - quoted strings: " ... "
    const_pattern = r'(?:[a-z][a-zA-Z0-9_]*|"[^"]*")'

    pattern = re.compile(
        rf'\b(pos|neg)\s*\(\s*eqo\s*\(\s*({const_pattern})\s*,\s*({const_pattern})\s*\)\s*\)'
    )
    ...
    for kind, const1, const2 in matches:
        if kind == "pos":
            pos_facts.append((const1, const2))
        else:
            neg_facts.append((const1, const2))

    indexed_pos = {
        f'pos(eqo({c1},{c2}),{idx}).'
        for idx, (c1, c2) in enumerate(pos_facts)
    }

    indexed_neg = {
        f'neg(eqo({c1},{c2}),{idx}).'
        for idx, (c1, c2) in enumerate(neg_facts)
    }
    return indexed_pos, indexed_neg
