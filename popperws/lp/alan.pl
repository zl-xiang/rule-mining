...
% sim_thresh(90).
% sim_thresh(95).
% vars(3,(V1,V2,Theta)) :- vars(2,(V1,V2)), sim_thresh(Theta).
% var_pos(V1,(V1,V2,Theta),0) :- vars(3,(V1,V2,Theta)), sim_thresh(Theta).
% var_pos(V2,(V1,V2,Theta),1) :- vars(3,(V1,V2,Theta)), sim_thresh(Theta).
% const_pos(Theta,(V1,V2,Theta),2) :- vars(3,(V1,V2,Theta)), sim_thresh(Theta).
% ordered_vars((V1,V2,Theta),(0,V2,Theta))
% Generate whatever
% Similarity joins
{body_literal(0,sim,3,(Svar1,Svar2,Theta))}:-
    ...
    compatible_attr(Attr1,Attr2), Svar1 != Svar2, sim_thresh(Theta).
...
%%% [20260402] modified
bound(C,Var1) :- body_literal(C,sim,3,(Var1,Var2,_)), body_literal(C,att,3,(_,_,Var1)).
bound(C,Var2) :- body_literal(C,sim,3,(Var1,Var2,_)), body_literal(C,att,3,(_,_,Var2)).
...
:- body_literal(C,sim,3,Vars), var_member(Var,Vars), not bound(C,Var).
...
%:- body_literal(C,R,1,Tid),  body_literal(C,sim,3,Vars1), var_member(Tid,Vars1).
% [be careful here!] tid may collide with threshold if threshold number is included as var accidentally
:- body_literal(C,R,1,Tid),  body_literal(C,sim,3,Vars1), var_member(Tid,Vars1).
...
%%% [20260402] modified
:- body_literal(0,sim,3,(X,Y,_)), body_literal(0,sim,3,(Y,X,_)).
%%% [20260402] modified
%% no other similarity literals with different threshold
:- body_literal(0,sim,3,(X,Y,Theta1)), body_literal(0,sim,3,(X,Y,Theta2)), Theta1!=Theta2.

:- body_literal(0,sim,3,(X,Y,Theta)), body_literal(C,att,3,(Tid1,Attr1,X)), body_literal(C,att,3,(Tid2,Attr2,Y)), not sim_defined(Attr1,Attr2).
...
:- body_literal(C,att,3,(Tid1,Attr,S1)), body_literal(C,att,3,(Tid2,Attr1,S2)), body_literal(C,sim,3,(S1,S2,_)), attr(Attr,2).
...
:- body_literal(_, sim, _, (V0,V1, _)), singleton(V0).
:- body_literal(_, sim, _, (V0,V1, _)), singleton(V1).
