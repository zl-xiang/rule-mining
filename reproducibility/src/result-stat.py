...
def plot_dc_acc(ver=""):
    ...
    titles = ['Dblp-DC', 'Cora-DC', 'Imdb-DC', 'Poke-DC']
    titles = [r'\textit{' + t + '}' for t in titles]
    paths = []
    ver = "-"+ ver if len(ver)>0 else ver
    x_labels = []
    for i,n in enumerate(names):
        ...
        _ver = "" if n not in ['cora','imdb'] else ver
        path = [ f'./lp/{names[i]}/{p}/results{_ver}/scores/{names[i]}-{p}-s1-dc-scores3.csv' for p in split]
        paths.append(path)
        # title = r'\textit{' + title + '}'
    plot_acc_from_csv_groups(csv_groups=paths, variance='sem',line_labels=["\\textbf{Precision}", "\\textbf{Recall}", "\\textbf{F1}"], columns=['test_precision','test_recall','test_f1',],group_titles=titles,save_path=f'./lp/acc-var-all-dc.pdf', x_labels_groups=x_labels)
...
def plot_acc(ver=""):
    ...
    for i,n in enumerate(names):
        ...
        x_labels.append(labels)
        _ver = "" if n not in ['cora','imdb'] else ver
        path = [ f'./lp/{names[i]}/{p}/results{_ver}/scores/{names[i]}-{p}-s1-scores.csv' for p in split]
        paths.append(path)
        # title = r'\textit{' + title + '}'
    plot_acc_from_csv_groups(csv_groups=paths, variance='sem',line_labels=["\\textbf{Precision}", "\\textbf{Recall}", "\\textbf{F1}"], columns=['test_precision','test_recall','test_f1',],group_titles=titles,save_path=f'./lp/acc-var-{ver}-all.pdf', x_labels_groups=x_labels)
...
def plot_rulesize(name,ver=""):
    split = [80, 0.8,0.6,0.4,0.2]
    labels = [100, 80,60,40,20]
    ver = "-"+ver if len(ver) >0 else ver
    path = [ f'./lp/{name}/{p}/results{ver}/scores/{name}-{p}-s1-scores.csv' for p in split]
    plot_size_from_csvs(csv_paths=path, variance='sem', columns=['rules'],title=f'{name} rule size',save_path=f'./lp/{name}/{name}{ver}-rulesize.pdf', labels = labels)
    
def plot_runningtime(name, title, ver=""):
    split = [80, 0.8,0.6,0.4,0.2]
    labels = [100, 80,60,40,20]
    ver = "-"+ver if len(ver) >0 else ver
    path = [ f'./lp/{name}/{p}/results{ver}/scores/{name}-{p}-s1-scores.csv' for p in split]
    plot_metrics_from_csvs(csv_paths=path, variance='sem', columns=['popper_time_sec'],title=r'\textit{' + title + '}',save_path=f'./lp/{name}/{name}{ver}-time.pdf', labels = labels)
...
if __name__ == "__main__":
    ...
    pathno = './lp/pokemon/80/results-no-optim/scores/pokemon-80-s1-scores.csv'
    path2 = './lp/pokemon/80/resultspw/scores/pokemon-80-s1-scores.csv'
    pathc = './lp/cora/80/results-trans2/scores/cora-80-s1-scores.csv'
    pathcno = './lp/cora/80/results-no-optim/scores/cora-80-s1-score.csv'

    pathd = './lp/dblp-scholar/80/resultst-no-optim/scores/dblp-scholar-80-s1-scores.csv'
    pathd2 = './lp/dblp-scholar/80/results/scores/dblp-scholar-80-s1-scores.csv'
    ipath = './lp/imdb/80/results-trans2/scores/imdb-80-s1-scores.csv'
    ipathno = './lp/imdb/80/results-no-optim/scores/imdb-80-s1-scores.csv'
    ipathpw = './lp/imdb/80/resultspw-trans2/scores/imdb-80-s1-scores.csv'
    # path = './lp/imdb/80/resultspw/scores/imdb-80-s1-scores.csv'
    # plot_dc_bars()
    #print(compute_stats(path))
    ...
    # plot_rulesize('imdb',ver="trans2")
    # plot_acc(ver="trans2")
    # plot_dc_acc(ver="trans2")
