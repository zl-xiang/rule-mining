...
base_dir="./results-no-optim"

mkdir -p "$base_dir/logs"
mkdir -p "$base_dir/rules"
mkdir -p "$base_dir/scores"

score_file="$base_dir/scores/${dataset}-${split}-s${sample}-score.csv"
...
do
    ...
    python /home/zhiliangxiang/Academic/project/rule-mining/rule-mining/popper.py -fp "$theta" -d "$delta" --no-combine ./s${s} \
        > "$raw_log" 2>&1
