...
base_dir="./results"
score_file="$base_dir/scores/${dataset}-${split}-s${sample}-dc-scores2.csv"

# Create CSV header
if [ ! -f "$score_file" ]; then
    echo "sample,sub_sample,test_precision,test_recall,test_f1" \
        > "$score_file"
fi
...
do
    ...
    do
        ...
        raw_log="$base_dir/logs/dc/${run_name}-dc.log"
        ...
        era_log="${era_output_dir}/${run_name}-dc2.log"
        ...
        echo "${s},${v},${test_precision},${test_recall},${test_f1}" \
            >> "$score_file"
