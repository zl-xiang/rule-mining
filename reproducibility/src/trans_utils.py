...
def get_merge_attributes_local_trc(dir:str):
    rule_list:list[str] = get_rule_list(dir)[0]
    merge_o_attrs = dict()
    merge_v_attrs = dict()
    #print(rule_list)
    ...
    for r in rule_list:
        ...
        # print(h)
        # get merge relations
        if h.startswith(EQO_PRED) or h.startswith(ACTIVE_O_PRED) or h.startswith(EQ_PRED) or h.startswith(ACTIVE_PRED):
            #print(merge_vars)
            # iterate rule body to locate occurring relation
            x = locate_trc_body_var(merge_vars[0],b)
            print(x)
            y = locate_trc_body_var(merge_vars[1],b)
            print(y)
            if x != None and y != None:
                if x[0] not in merge_o_attrs:
                    merge_o_attrs[x[0]] = set()
            ...
            t1_rel = locate_trc_body_var(merge_vars[0],b)[0]
            t2_rel = locate_trc_body_var(merge_vars[2],b)[0]
            pos1 = merge_vars[1]
            pos2 = merge_vars[3]
            if t1_rel not in merge_v_attrs:
                merge_v_attrs[t1_rel] = set()
            merge_v_attrs[t1_rel].add(pos1)
            if t2_rel not in merge_v_attrs:
                merge_v_attrs[t2_rel] = set()
            merge_v_attrs[t2_rel].add(pos2)
    # print(merge_o_attrs)
    # merge_attrs.pop(EQ_PRED)
    return merge_o_attrs, merge_v_attrs    
...
def get_merge_local_pairs(dir:str):
    rule_list:list[str] = get_rule_list(dir)[0]
    merge_v_attrs = set()
    #print(rule_list)
    # check rule head variables
    for r in rule_list:
        ...
        b = r[1]
        merge_vars = get_atom_vars(h)
        if h.startswith(EQV_PRED) or h.startswith(ACTIVE_V_PRED): 
            pos1 = locate_body_var(merge_vars[0],b)
            pos2 = locate_body_var(merge_vars[2],b)
            
            if ((pos2[0],int(merge_vars[3])),(pos1[0],int(merge_vars[1]))) not in merge_v_attrs:
                merge_v_attrs.add(((pos1[0],int(merge_vars[1])),(pos2[0],int(merge_vars[3]))))             
    # merge_attrs.pop(EQ_PRED)
    return merge_v_attrs  
